...
const PROVINCE_POSTAL_PREFIX: Record<number, string[]> = {
  1: ["8"], 2: ["3"], 3: ["1", "4"], 4: ["3"], 5: ["5"],
  6: ["1"], 7: ["9"], 8: ["3"], 9: ["1", "4"], 10: ["5"],
  11: ["6"], 12: ["7"], 13: ["7"], 14: ["7"], 15: ["7"],
  16: ["7"], 17: ["2"], 18: ["3"], 19: ["9"], 20: ["9"],
  21: ["2"], 22: ["8"], 23: ["8"], 24: ["9"], 25: ["9"],
  26: ["2"], 27: ["9"], 28: ["9"], 29: ["9"], 30: ["9"],
  31: ["9"], 32: ["2"], 33: ["3"], 34: ["2"],
};
...
/* ── Helper: deduplicate select items by ID field ──
   Prevents React duplicate-key warning when API returns items
   with the same or undefined ID. */
...
export default function AddressPage() {
  ...
  const form = useForm<AddressFormValues>({
    ...
    defaultValues: {
      ...
      subdistrict_name: "", postal_code: "", is_default: false,
    },
  });
  ...
  const onSubmit = async (data: AddressFormValues) => {
    const user = getAuthUser();
    if (!user) { toast.error("Sesi telah habis, silakan login kembali."); return; }
  ...
  /* ── Deduped select lists ── */
  const uniqueProvinces = dedupeById(provinces, "province_id");
  const uniqueCities    = dedupeById(cities, "city_id");
  const uniqueDistricts = dedupeById(subdistricts, "subdistrict_id");
                ...
                >
                  <div className="space-y-3 flex-1">
                    ...
                    <div className="space-y-1.5 text-sm text-muted-foreground">
                      ...
                      <p className="leading-relaxed">
                        {address.address}<br />
                        {address.subdistrict_name}, {address.district_name}<br />
                  ...
                  <div className="hidden sm:flex flex-col items-end gap-3 justify-start shrink-0">
                    <div className="flex gap-2">
                      <Button variant="outline" size="sm" onClick={() => handleOpenEdit(address)} className="rounded-full text-xs h-8">
                        <Pencil className="h-3.5 w-3.5 mr-1.5" />Edit
                      </Button>
                      {!address.is_default && (
                        <Button variant="outline" size="sm" onClick={() => confirmDeleteAddress(address)} className="rounded-full text-xs h-8 text-destructive border-destructive/20 hover:bg-destructive/10 hover:text-destructive">
                          <Trash2 className="h-3.5 w-3.5 mr-1.5" />Hapus
                        </Button>
                      )}
                    </div>
                    {!address.is_default && (
                      <Button variant="ghost" size="sm" className="rounded-full text-xs h-8 text-primary hover:bg-primary/10" onClick={() => handleSetDefault(address)}>
                        Jadikan Utama
                      </Button>
                    )}
                  </div>
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <FormField control={form.control} name="province_id" render={({ field }) => (
                    <FormItem>
                      ...
                      <Select
                        onValueChange={(v) => { field.onChange(v); form.setValue("city_id", ""); form.setValue("district_id", ""); }}
                        value={field.value}
                      >
                  ...
                  <FormField control={form.control} name="city_id" render={({ field }) => (
                    <FormItem>
                      ...
                      <Select
                        onValueChange={(v) => { field.onChange(v); form.setValue("district_id", ""); }}
                        value={field.value}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      ...
                      <Select onValueChange={field.onChange} value={field.value} disabled={!watchCity}>
                        ...
                        <SelectContent>
                          {uniqueDistricts.map((d) => (
                            <SelectItem key={`dist-${d.subdistrict_id}`} value={String(d.subdistrict_id)}>
                              {d.subdistrict_name}
                            </SelectItem>
                          ))}
                        </SelectContent>
                  ...
                  <FormField control={form.control} name="subdistrict_name" render={({ field }) => (
                    <FormItem>
                      <FormLabel>Kelurahan</FormLabel>
                      <FormControl><Input placeholder="Cth: Kelurahan Kuta" className="h-11 rounded-xl" {...field} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <FormField control={form.control} name="postal_code" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input placeholder="12345" className="h-11 rounded-xl" {...field} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                </div>
