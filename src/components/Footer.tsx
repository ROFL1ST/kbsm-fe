      ...
      <div className="container">
        <div className="grid gap-10 border-b border-background/10 pb-16 lg:grid-cols-12">
          <div className="space-y-5 lg:col-span-4">
            <Link to="/" className="inline-block font-display text-3xl font-semibold">
          ...
          <div className="grid grid-cols-2 gap-8 md:grid-cols-3 lg:col-span-8">
            <div>
              <h4 className="mb-4 font-display text-base text-primary-glow">Shop</h4>
              {isCategoriesError ? (
                <p className="text-sm text-background/60">Gagal memuat kategori.</p>
              ) : (
                <div className={`grid gap-6 ${shouldSplitCategories ? "grid-cols-1 sm:grid-cols-2" : "grid-cols-1"}`}>
                  {isCategoriesLoading && categories.length === 0
                    ? Array.from({ length: shouldSplitCategories ? 2 : 1 }).map((_, columnIndex) => (
                        <ul key={columnIndex} className="space-y-3">
                          {Array.from({ length: 4 }).map((_, index) => (
                            <li key={index} className="h-4 w-24 animate-pulse rounded bg-background/10" />
                          ))}
                        </ul>
                      ))
            ...
            {INFO_COLUMNS.map((column) => (
              <div key={column.title}>
                <h4 className="mb-4 font-display text-base text-primary-glow">{column.title}</h4>
        ...
        <div className="grid gap-6 border-b border-background/10 py-8 text-xs md:grid-cols-2">
          <div>
            <p className="mb-3 uppercase tracking-[0.2em] text-background/40">We Accept (bank)</p>
          ...
          <div className="md:text-right">
            <p className="mb-3 uppercase tracking-[0.2em] text-background/40">Shipping Partners</p>
