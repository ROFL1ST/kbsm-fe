...
/* ── Compact slide info (single column: label → title → image → price → timer → cta) ── */
        ...
        {total > 1 && (
          <div className="flex items-center gap-3">
            <Button
              asChild
              size="default"
              className="rounded-full bg-foreground text-background hover:bg-primary uppercase elegant-shadow group h-11 px-5 text-xs tracking-[0.15em]"
            >
...
/* ── Full slide info (Index page — layout lama tidak berubah) ─ */
const SlideInfo = ({
  item, visible,
  prev, next, total, active, goTo,
}: {
    ...
    >
      <div className="space-y-7">
        ...
        <div className="flex items-baseline gap-3">
          <span className="font-display font-semibold text-foreground text-3xl">
            {formatRupiah(item.final_price)}
          </span>
        ...
        {total > 1 && (
          <div className="flex items-center gap-3">
            <Button
              asChild
              size="lg"
              className="rounded-full bg-foreground text-background hover:bg-primary uppercase elegant-shadow group h-14 px-8 text-sm tracking-[0.15em]"
            >
              <Link to={`/shop/product/${item.product_unit_id}`}>
                Shop Now
...
const FullDiscountSlider = () => {
  ...
  return (
    <div
      ...
      onMouseLeave={() => setPaused(false)}
    >
      {/* Gambar — horizontal slide */}
      <div className="relative overflow-hidden rounded-3xl luxury-shadow aspect-square lg:aspect-[4/5]">
        <div
