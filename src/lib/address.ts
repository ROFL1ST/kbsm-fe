...
/* ── Normalise helpers ── */
const toProvince = (region: RawRegion): ApiProvince => ({
  province_id: region.id,
  province: region.name,
});

const toCity = (region: RawRegion): ApiCity => ({
  city_id: region.id,
  city_name: region.name,
});

const toSubdistrict = (region: RawRegion): ApiSubdistrict => ({
  subdistrict_id: region.id,
  subdistrict_name: region.name,
});

const toDistrict = (district: RawDistrict): ApiDistrict => ({
...
export async function getUserAddresses(userId: string): Promise<ApiAddress[]> {
  const payload = await fetchAuth<ApiAddress[]>(
    `/auth/user-addresses?user_id=${userId}`
  );
...
export async function getDefaultUserAddress(
  ...
  const payload = await fetchAuth<ApiAddress | ApiAddress[]>(
    `/auth/user-addresses?user_id=${userId}&is_default=true`
  );
...
export async function deleteUserAddress(
  ...
  if (!(payload as { status?: boolean }).status) {
    throw new Error(
      (payload as { message?: string }).message || "Gagal menghapus alamat."
    );
  }
}
...
export async function getApiProvinces(): Promise<ApiProvince[]> {
  const payload = await fetchAuth<RawRegion[]>("/auth/provinces", {
    method: "GET",
  });
...
export async function getApiCities(
...
export async function getApiSubdistricts(
...
export async function getApiDistricts(
